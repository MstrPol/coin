...
| Bootstrap | `gitea-git`, clone product repo, executor URL в manifest |
| Pod | K8s cloud, `make endpoints`, образ `manifest.runtime.image` |
| Stages | логи `coin-executor run`, content sha256 |
...
**Симптом:** validate падает на hash content.

**Действия:**

```bash
cd docker && make coin-jenkins-agents   # re-seed catalog + Nexus cache
# rebuild coin-api если изменился store.go expected hash
docker compose up -d --build coin-api
```
