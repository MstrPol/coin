...
Coin всегда ищет последний тег вида `*-[rc|snapshot]-N` в данной серии (`major.minor.patch-JIRA-ID`) и увеличивает N на 1. Первый тег получает N=1.

**Правила:**

- **`rc`** выпускается **только с ветки `release/*`**. Coin отклонит запрос с любой другой ветки.
- **`snapshot`** выпускается с любой ветки. Не является релизным артефактом, не деплоится на прод.
- `major.minor.patch` у snapshot берётся из последнего RC-тега на текущей ветке; если тегов нет — `0.0.0`.
...
| Ветка / тег | `COIN_VERSION` | Публикация в Nexus |
|---|---|---|
| `release/PROJ-404` + RC-тег | `1.5.0-PROJ-404-rc-N` | ✅ да |
| `release/PROJ-404` без тега | `1.5.0-PROJ-404-snapshot-1`, `-snapshot-2`, … | ❌ нет |
| `main` | `0.0.0-main-snapshot-1`, `-snapshot-2`, … | ❌ нет |
| `feature/PROJ-101` | `0.0.0-PROJ-101-snapshot-1`, `-snapshot-2`, … | ❌ нет |
| `bugfix/PROJ-202` | `0.0.0-PROJ-202-snapshot-1`, `-snapshot-2`, … | ❌ нет |

> `1.5.0` у `release/PROJ-404` берётся из последнего RC-тега данной ветки.  
> До первого `coinRelease` — будет `0.0.0`.
...
Запустить coinRelease
  ...
  ├── Ветка:       release/PROJ-404     ← только release/*
  ├── Base:        1.5.0
  │
  Coin:
  1. Проверяет, что HEAD на ветке release/*   ← не release/* → ошибка
  2. Ищет последний тег v1.5.0-PROJ-404-rc-N
  3. Вычисляет N+1 → v1.5.0-PROJ-404-rc-2
...
| Ситуация | base | Тег |
|---|---|---|
| Первый релиз | `1.0.0` | `v1.0.0-PROJ-404-rc-1` |
| Новая фича (minor) | `1.1.0` | `v1.1.0-PROJ-500-rc-1` |
| Breaking change (major) | `2.0.0` | `v2.0.0-PROJ-600-rc-1` |
| Срочный патч | `1.0.1` | `v1.0.1-PROJ-505-rc-1` |

*Ориентир: «Потребителю надо что-то менять у себя? → major. Просто получит новое? → minor. Вообще не заметит? → patch.»*

### Типичные сценарии

| Ситуация | Ветка | Base → тег |
|---|---|---|
| Первая отдача на ПСИ | `release/PROJ-404` | `1.5.0` → `v1.5.0-PROJ-404-rc-1` |
| Фикс по итогам ПСИ | `release/PROJ-404` | `1.5.0` → `rc-2`, `rc-3`, … |
| Срочный фикс прода | `release/PROJ-505` | `1.5.1` → `v1.5.1-PROJ-505-rc-1` |
