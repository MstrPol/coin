...
Каждое замечание = отдельный коммит + **новый тег**:

```
v1.5.0 → ПСИ → замечание → фикс → v1.5.1 → ПСИ повторно
v1.5.1 → ПСИ → замечание → фикс → v1.5.2 → ПСИ → ПРИНЯТО
```

Каждый патч-тег = отдельная сборка Coin = отдельный артефакт в registry. Нельзя «пересобрать» тот же тег.
...
| Фикс по итогам ПСИ | `release/PROJ-404` | `patch` |
| Hotfix на прод | `hotfix/1.5.3` | `patch` |
