...
Переходы: `draft` → `canary` (publish to canary) → `published` (promote after health gate).
...
| # | Вопрос | Решение |
|---|--------|---------|
| Q1 | `component_artifact_bodies` | **PG только для draft Studio;** published — Nexus-only |
