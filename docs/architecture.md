...
|------|-----------|----------|
| `agent` | `agent/coin-agent` | `runtime.image`; `executor` derived от agent |
| `gp-content` | `gp-content/{stack}` | `build`, `pipeline`, `validateSchema`, `capabilities` |
| `branching-model` | `branching-model/{name}` | `branching` |

**Не в GP composition map:** `executor` (materialized из agent stack), `lib` (platform pin для Jenkins `@Library`).
