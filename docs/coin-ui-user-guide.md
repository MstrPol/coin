...
**Editors:** gp-content / branching-model — `/platform/.../:name/:version/edit` (validate → register → promote). Branching-model editor: schema v2 rule cards + YAML/card mapping; runtime branching behavior is evaluated by `coin-executor` during CI. Agent — metadata catch-up `/platform/runtime/:name/:version/edit` (image + digest; tag MUST match version); **New draft** — только Image ref + Digest (version из тега); CI path через `publish-agent.sh` (draft register only) → promote на release detail.

**Release detail:** `/platform/{family}/:name/releases/:version` — для agent: metadata `image` + `digest`. Draft agent: **Publish** и **Delete draft** (publisher+); список releases — **Delete** на строке draft. Draft branching-model: **Delete** на Releases tab и **Delete draft** в editor lifecycle panel.
