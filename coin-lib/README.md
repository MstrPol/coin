...
`coinPodYaml` подставляет в [`coin-pod-template.yaml`](resources/coin-pod-template.yaml) образ `runtime.image`, env `COIN_BUILD_ENGINE`, emptyDir 12Gi для podman storage.
...
2. **buildpack:** `podman load` Paketo builder из `/usr/share/coin/paketo-builder.tar` (если нужно)
