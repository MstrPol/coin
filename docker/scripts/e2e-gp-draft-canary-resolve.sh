...
GP_DRAFT="${COIN_GP_DRAFT_VERSION:-1.0.0-snapshot.8888}"
...
api_post "/v1/admin/components" "$(jq -n --arg n "${GC_NAME}" --arg a "${ACTOR}" \
  '{type: "gp-content", name: $n, actor: $a}')" >/dev/null
api_post "/v1/admin/components/gp-content/${GC_NAME}/versions/drafts" \
  "$(jq -n --arg v "${GC_DRAFT}" --arg a "${ACTOR}" '{version: $v, actor: $a}')" >/dev/null

gc_status="$(curl -sS "${API}/v1/admin/components/gp-content/${GC_NAME}/versions/${GC_DRAFT}" "${AUTH[@]}" | jq -r '.status // empty')"
...
echo "==> promote GP draft (expect 409 — draft gp-content pin)"
promote_code="$(curl -sS -o /tmp/coin-e2e-gp-promote.json -w '%{http_code}' -X POST \
  "${API}/v1/admin/golden-paths/${GP}/versions/${GP_DRAFT}/promote?actor=${ACTOR}" "${AUTH[@]}" -d '{}')"
if [[ "${promote_code}" != "409" ]]; then
  echo "FAIL: expected promote HTTP 409, got ${promote_code}: $(cat /tmp/coin-e2e-gp-promote.json)" >&2
  exit 1
fi
if ! jq -e '.blockingPins | length > 0' /tmp/coin-e2e-gp-promote.json >/dev/null; then
  echo "FAIL: expected blockingPins in promote response" >&2
  exit 1
fi
echo "OK: promote blocked by draft gp-content pin"

catalog_code="$(curl -sS -o /tmp/coin-e2e-catalog.json -w '%{http_code}' -X PATCH \
  "${API}/v1/admin/golden-paths/${GP}/catalog" "${AUTH[@]}" \
  -d "$(jq -n --arg stable "${GP_STABLE}" --arg canary "${GP_DRAFT}" --arg a "${ACTOR}" \
    '{latest: $stable, latestCanary: $canary, minimum: $stable, actor: $a}')")"
if [[ "${catalog_code}" != "200" ]]; then
  echo "SKIP: catalog PATCH HTTP ${catalog_code} ($(cat /tmp/coin-e2e-catalog.json)) — API rejects snapshot/draft on latestCanary"
  echo "OK: GP draft create + promote gate (draft gp-content pin)"
  exit 0
fi
...
if [[ "${resolved}" != "${GP_DRAFT}" && "${resolved}" != "$(echo "${GP_DRAFT}" | sed 's/-snapshot\.[^.]*$//')" ]]; then
