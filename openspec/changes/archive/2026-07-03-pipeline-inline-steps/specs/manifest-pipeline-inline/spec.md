...
Resolved manifest SHALL contain `parameters`, `validateSchema` ref, and `pipeline.stages` with inline steps. Buildkit steps in manifest MUST include co-located `containerfile` with `contentRef` and `digest` on the same step object. Manifest MUST NOT contain top-level `build.targets`, `deliverables`, or `artifacts.containerfiles`.

#### Scenario: Materialize go-app v3 manifest

- **WHEN** coin-api resolves GP with pipeline-inline gp-content
...
#### Scenario: Manifest hash covers step containerfile

- **WHEN** publisher changes `containerfile.body` on a pipeline step
