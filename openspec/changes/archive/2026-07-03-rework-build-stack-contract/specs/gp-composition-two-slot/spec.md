...
2. **`gp-content`** — Build Stack vNext package: parameters, targets, deliverables, Containerfile artifacts, schema and pipeline contract
...
- **AND** MUST contain Build Stack vNext `parameters`, `build.targets`, `pipeline`, `deliverables`, `artifacts` and validation schema materialized from the `gp-content` pin
