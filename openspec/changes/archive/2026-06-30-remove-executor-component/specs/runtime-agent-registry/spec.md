...
**Reason**: Agent component is the sole CI runtime stack; executor binary is baked in the agent image. Separate registry row and manifest section are redundant and cause false GP promote failures.

**Migration**: Remove `executor` components from PG; re-resolve GP manifests; stop registering executor in coin-api.
